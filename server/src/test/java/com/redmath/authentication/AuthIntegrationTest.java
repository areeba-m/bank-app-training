...
class AuthIntegrationTest {
    ...
    @Test
    void register_withNewEmail_createsAccountAndInitialBalance() throws Exception {
        ...
        assertThat(account.getBalance().getAmount()).isEqualByComparingTo(BigDecimal.ZERO);
        assertThat(account.getBalance().getIndicator()).isEqualTo(Indicator.CR);
    ...
    void register_withDuplicateEmail_returns4xx() throws Exception {
        ...
        RegisterRequest request = new RegisterRequest("Another User", EMAIL,
                "AnotherPassword123!", "Another Address");
    ...
    void login_withValidCredentials_returnsTokens() throws Exception {
        ...
        RefreshToken refreshToken = refreshTokenRepository.findByToken(Objects.requireNonNull(refreshCookie).getValue())
                .orElseThrow();
    ...
    @Test
    void refresh_withValidToken_rotatesRefreshToken() throws Exception {

        registerUser();
        Cookie oldCookie = getRefreshCookie();
        String oldToken = Objects.requireNonNull(oldCookie).getValue();
        ...
        String newToken = Objects.requireNonNull(newCookie).getValue();
    ...
    @Test
    void refresh_withOldTokenAfterRotation_returnsBadRequest() throws Exception {
    ...
    void cleanupExpiredTokens_removesOnlyExpiredTokens() {
        ...
        Account expiredUser = createAccount("Expired User", "expired@example.com");

        Account validUser = createAccount("Valid User", "valid@example.com");
    ...
    @Test
    void verifyOtp_withValidOtp_returnsPasswordResetToken() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post(BASE + "/otp/verify")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(request))
                .andDo(print())
        ...
        String resetToken = objectMapper.readTree(result.getResponse().getContentAsString())
    ...
    @Test
    void verifyOtp_withExpiredOtp_returnsUnauthorized() throws Exception {
        ...
        token.setCreatedAt(Instant.now().minus(2, ChronoUnit.DAYS));
        ...
        OtpToken savedToken = otpTokenRepository.findById(token.getId()).orElseThrow();
        assertThat(savedToken.isUsed()).isFalse();
    }

    @Test
    void verifyOtp_cannotReuseAlreadyUsedOtp() throws Exception {
    ...
    @Test
    void resendOtp_generatesNewOtpAndInvalidatesOldOtp() throws Exception {
        ...
        reset(emailService);
        doNothing().when(emailService).sendOtpEmail(anyString(), anyString(), anyString(), anyLong());
        ...
        OtpToken updatedOldToken = otpTokenRepository.findById(oldTokenId).orElseThrow();
        assertThat(updatedOldToken.isUsed()).isTrue();

        OtpToken newToken = otpTokenRepository
                .findTopByAccountAndUsedFalseOrderByCreatedAtDesc(account)
                .orElseThrow();
        assertThat(newToken.getId()).isNotEqualTo(oldTokenId);

        verify(emailService).sendOtpEmail(anyString(), anyString(), anyString(), anyLong());
    }
    ...
    @Test
    void changePassword_withValidResetToken_changesPassword() throws Exception {
        ...
        String oldEncodedPassword = accountRepository.findByEmail(EMAIL).orElseThrow().getPassword();

        String otp = generateAndCaptureOtp(account);
        String resetToken = verifyOtpAndGetResetToken(otp);

        SetNewPasswordRequest request = new SetNewPasswordRequest(resetToken, NEW_PASSWORD);
        ...
        Account updatedAccount = accountRepository.findByEmail(EMAIL).orElseThrow();

        assertThat(updatedAccount.getPassword()).isNotEqualTo(oldEncodedPassword);
        assertThat(passwordEncoder.matches(NEW_PASSWORD, updatedAccount.getPassword())).isTrue();
        assertThat(passwordEncoder.matches(PASSWORD, updatedAccount.getPassword())).isFalse();

        assertThat(updatedAccount.isPasswordChangeRequired()).isFalse();

        assertThat(refreshTokenRepository.findByToken(Objects.requireNonNull(refreshCookie).getValue())).isEmpty();
    ...
    void changePassword_withSamePassword_returnsBadRequest() throws Exception {
        ...
        SetNewPasswordRequest request = new SetNewPasswordRequest(resetToken, PASSWORD);
    ...
    @Test
    void changePassword_withInvalidResetToken_doesNotChangePassword() throws Exception {

        registerUser();
        Account beforeChange = accountRepository.findByEmail(EMAIL).orElseThrow();
        String oldPasswordHash = beforeChange.getPassword();

        SetNewPasswordRequest request = new SetNewPasswordRequest("invalid-reset-token", NEW_PASSWORD);
        ...
        Account afterChange = accountRepository.findByEmail(EMAIL).orElseThrow();

        assertThat(afterChange.getPassword()).isEqualTo(oldPasswordHash);
        assertThat(passwordEncoder.matches(PASSWORD, afterChange.getPassword())).isTrue();
        assertThat(passwordEncoder.matches(NEW_PASSWORD, afterChange.getPassword())).isFalse();
    }
    ...
    void logout_deletesRefreshTokenAndClearsCookie() throws Exception {
        ...
        String refreshToken = Objects.requireNonNull(refreshCookie).getValue();
    ...
    private Account registerUser() throws Exception {
        ...
        return accountRepository.findByEmail(AuthIntegrationTest.EMAIL).orElseThrow();
    }
    ...
    private Cookie getRefreshCookie() throws Exception {
        return (Cookie) loginAndGetResult().get("cookie");
    }
    ...
    private String generateAndCaptureOtp(Account account) {
        ...
        accountRepository.findById(account.getUserId()).orElseThrow();
        ...
        otpTokenRepository.findTopByAccountOrderByCreatedAtDesc(account)
                .ifPresent(token -> {
                    token.setCreatedAt(Instant.now().minus(2, ChronoUnit.MINUTES));
                    otpTokenRepository.save(token);
                });
        ...
        verify(emailService).sendOtpEmail(anyString(), anyString(), otpCaptor.capture(), anyLong());

        return otpCaptor.getValue();
    }

    private String verifyOtpAndGetResetToken(String otp) throws Exception {
