...
import {
    ArrowLeft, Mail, MapPin, Key, Receipt, PlusCircle,
    Shield, Calendar
} from 'lucide-react';
...
export const AccountDetailView = () => {
    ...
    const { selectedAccount: account } = useSelector((state) => state.accounts);
    const { balanceInfo, list: transactions } = useSelector((state) => state.transactions);
    ...
    const handleExecuteTransaction = async (accId, txnData) => {
        await dispatch(executeTransaction({ accountId: accId, transactionData: txnData })).unwrap();
        await loadData();
    };

    if (loading) {
        return <div className="p-8 text-center text-slate-400 text-xs">Loading statement details...</div>;
    }

    if (!account) {
        return (
            <div className="p-8 space-y-4">
                <p className="text-xs font-semibold text-rose-700">Account not found.</p>
                <button onClick={() => navigate('/admin/dashboard')} className="text-xs font-bold text-burgundy-700 underline">
                    Back to Directory
                </button>
            </div>
        );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-burgundy-100/80 shadow-xs grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">

                <div className="lg:col-span-7 space-y-3">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-burgundy-700 text-white flex items-center justify-center font-bold text-lg shadow-burgundy-glow">
                            {account.name.charAt(0)}
                        </div>
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className="text-lg font-extrabold text-slate-900">{account.name}</h1>
                                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-burgundy-50 text-burgundy-800 border border-burgundy-200">
                  {account.role}
                </span>
                            </div>
                            <p className="text-[11px] text-slate-400 font-mono">ID: {account.id}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-600 pt-2 border-t border-slate-100">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="truncate">{account.address || 'N/A'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Key className="w-3.5 h-3.5 text-burgundy-600 shrink-0" />
                            <span className="font-mono">Pass: {account.password}</span>
                        </div>
                        <div className="flex items-center gap-2">
                ...
                <div className="lg:col-span-5 bg-gradient-to-br from-burgundy-900 via-burgundy-800 to-burgundy-950 p-5 rounded-2xl text-white shadow-burgundy-lg">
                    <div className="flex justify-between items-center mb-3">
                        <span className="text-[9px] uppercase font-bold tracking-widest text-burgundy-200">Current Balance</span>
              ...
              {balanceInfo?.dbCrIndicator || 'CR'}
                    ...
                    <p className="text-2xl font-extrabold font-mono tracking-tight text-white mb-2">
                        ${(balanceInfo?.amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </p>
                    <div className="flex items-center gap-1.5 text-[10px] text-burgundy-200 font-mono">
                        <Calendar className="w-3.5 h-3.5" />
                        <span>Date: {balanceInfo?.date || 'Today'}</span>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-burgundy-100/80 shadow-xs p-5 space-y-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-extrabold text-slate-900">Ledger Statement</h2>
                    </div>
                    <span className="text-[11px] text-slate-400 font-mono">{transactions.length} Records</span>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs text-slate-600">
                        ...
                        {transactions.length === 0 ? (
                            <tr>
                                <td colSpan="5" className="py-6 text-center text-slate-400">No transactions recorded.</td>
                            </tr>
                        ) : (
                            transactions.map((t) => (
