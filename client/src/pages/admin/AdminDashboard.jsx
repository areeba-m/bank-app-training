...
import {
    ...
    CheckCircle2, RefreshCw, AlertCircle, Building2
} from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-burgundy-900 via-burgundy-800 to-burgundy-950 rounded-3xl p-6 text-white shadow-burgundy-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight">Accounts Directory</h1>
                </div>
                ...
                <div className="bg-white p-4 rounded-2xl border border-burgundy-100/80 shadow-xs flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xl font-extrabold text-slate-900 font-mono">
                            ${totalSystemBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                        </p>
                    </div>
                </div>
            </div>
            <div className="bg-white rounded-3xl border border-burgundy-100/80 shadow-xs overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs text-slate-600">
                        ...
                        {loading ? (
                            <tr>
                                <td colSpan="7" className="py-6 text-center text-slate-400">Loading accounts...</td>
                            </tr>
                        ) : filteredAccounts.length === 0 ? (
                            <tr>
                                <td colSpan="7" className="py-6 text-center text-slate-400">No matching accounts found.</td>
                            </tr>
                        ) : (
                            filteredAccounts.map((acc) => (
                                <tr key={acc.id} className="hover:bg-sand-50/70 transition-colors">
                                    <td className="py-3 px-4 font-mono font-bold text-burgundy-900">{acc.id}</td>
                                    <td className="py-3 px-4 font-semibold text-slate-800">{acc.name}</td>
                                    <td className="py-3 px-4 text-slate-500">{acc.email}</td>
                                    <td className="py-3 px-4 text-slate-500 truncate max-w-xs">{acc.address || '—'}</td>
                                    <td className="py-3 px-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                          acc.role === 'ADMIN' ? 'bg-burgundy-700 text-white' : 'bg-slate-100 text-slate-700 border border-slate-200'
                      }`}>
                        {acc.role}
                      </span>
                                    </td>
                                    <td className="py-3 px-4 font-mono font-bold text-slate-900">
                                        ...
                                        <span className="ml-1 text-[10px] text-emerald-600 font-sans font-bold">({acc.dbCrIndicator || 'CR'})</span>
                                    </td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
                    <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-4 border border-rose-100">
                        ...
                        <p className="text-xs text-slate-600 leading-relaxed">
                            Are you sure you want to delete account <span className="font-mono font-bold text-slate-900">&apos;{deleteConfirmId}&apos;</span>?
                        </p>
