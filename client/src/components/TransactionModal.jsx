...
export const TransactionModal = ({ isOpen, onClose, onExecuteTransaction, accountId, currentBalance }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (type === 'WITHDRAW' && currentBalance !== undefined && numAmount > currentBalance) {
            setError(`Insufficient funds. Your current balance is $${currentBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}.`);
            return;
        }

        setLoading(true);
        try {
            await onExecuteTransaction(accountId, {
                description: description.trim() || (type === 'DEPOSIT' ? 'Deposit via Banking Portal' : 'Cash Withdrawal'),
                amount: numAmount,
                type
            });
            setAmount('');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-burgundy-100 overflow-hidden">
                <div className="bg-gradient-to-r from-burgundy-800 to-burgundy-950 px-6 py-4 flex items-center justify-between text-white">
                    ...
                    <button onClick={onClose} className="text-burgundy-200 hover:text-white">
                        <X className="w-5 h-5" />
                    </button>
                </div>
